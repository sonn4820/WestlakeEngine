...
{
	m_fmodSystem->set3DListenerAttributes(listenerIndex, ConvertVec3ToFmodVec(listenerPosition), ConvertVec3ToFmodVec(Vec3::ZERO), ConvertVec3ToFmodVec(listenerForward), ConvertVec3ToFmodVec(listenerUp));
}
...
SoundPlaybackID AudioSystem::StartSoundAt(SoundID soundID, const Vec3& soundPosition, bool isLooped, float volume, float balance, float speed, bool isPaused)
{
	...
	if (channelAssignedToSound)
	{
		...
		channelAssignedToSound->setPan(balance);
		channelAssignedToSound->setLoopCount(loopCount);
		channelAssignedToSound->set3DAttributes(ConvertVec3ToFmodVec(soundPosition), ConvertVec3ToFmodVec(Vec3::ZERO));
	}
...
FMOD_VECTOR* AudioSystem::ConvertVec3ToFmodVec(Vec3 const& vec3)
{
	FMOD_VECTOR* fmod = new FMOD_VECTOR();
	fmod->x = -vec3.y;
	fmod->y = vec3.z;
	fmod->z = -vec3.x;
	return fmod;
